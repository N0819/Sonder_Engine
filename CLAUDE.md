...
**Opening turn** (`turn.idx == 0`): `compile_world_context → director_establish → perception_establish → narrator → commit`
...
```
A plan step is one `steps`/`variants` row; the Director's five specialist calls are sub-calls *inside* `director_interpret`/`director_resolve`, not steps of their own. `compile_world_context` (`agents/mapping.py`) is deterministic: no `mapping` model role exists, and a beat that reaches for a room no plan holds raises a typed planning need (`world/planning_needs.py`) instead of inventing one.

Key ownership boundaries (see `AGENTS.md` for the full table):
- The **Director** (`agents/director.py`) owns objective causality — interprets player input and resolves outcomes — but not character psychology or narration, and must not silently replace the player's declared speech/action. It is no longer one mind: each Director stage fans out to a prose author plus five scoped specialists (`SPECIALISTS` in `agents/director_scopes.py` — `body`, `social`, `contact`, `objects`, `spatial`; the `offscreen` hand was retired 2026-09-04 and its traffic channels are `social`'s), each owning a subset of `state_diff` channels, and the deterministic orchestrator keeps every cross-channel judgment on the MERGED diff. The monolith is GONE: there is no `DEFAULT_PROMPTS["director_resolve"]`. `director_fanout_mode` chooses only CONCURRENCY (parallel default), never a different set of hands. Full contract in `AGENTS.md` § Director orchestration and design note 19.
...
Supporting service seams are explicit: `web/auth_routes.py` owns typed host-auth routes and cookie transport; `persist/chat_archive.py` owns portable chat import/export; `persist/pipeline_trace.py` owns privacy-conscious persisted-history export/replay; `world/spatial_orientation.py` owns bearing math. It is one of the `spatial_*` siblings behind the `world/spatial.py` FACADE, not a seam you import directly — `tools/project_check.py` enforces that for `world.spatial`, `agents.director` and `persist.commit` alike: a caller imports the facade, and only a test that PATCHES or introspects a sibling may name it.

Frontend (`static/js/`) uses browser globals, not ES modules. `theme-init.js` loads in the document head; the remaining order is `i18n-core.js → utils.js → components.js → editors.js → lorebooks.js → backdrops.js → ambience.js → weather-fx.js → chime.js → chat.js → writers_room.js → world_browser.js → settings.js → themes.js → extensions.js → app.js` (`static/index.html`). `i18n-core.js` is the localization rules, loaded by the SPA and by `login.html`/`guest.html` alike — there is one translator, not one per page. Never rename a shared JS function without grepping every file.
