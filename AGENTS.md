...
| Director orchestration (specialist scopes/gates, the six specialists, the parallel fan-out, the `director_orchestration` flag) | `agents/director.py` (`SPECIALISTS`, `_CHANNEL_GATES`, `_dispatch_specialists`, `_run_specialists`, `_orchestration_scope_backstop`), `prompts.py` (`SPECIALIST_PROMPT_SPECS`, `specialist_prompt`, the `_RESOLVE_*`/`RESOLVE_*` segments — the monolithic sheet is their byte-identical recomposition and the specialist chunks reuse the same segments) | `schemas.py` (`SPECIALIST_CHANNELS`, the `Director*Specialist` models, `orchestration` on both Director outputs), `providers.py` (`ROLE_FALLBACKS`, the `director_*` specialist roles), `tools/project_check.py` (`check_specialist_prompt_chunks`), design note 19, `tests/test_director_orchestration.py` — scope gates key on scene state and FAIL OPEN per channel; a channel shipping content outside every served scope must reach `tell_director`, never silence; the specialists are SHARED between interpret and resolve (one definition, two callers — interpret is not a lesser authority); specialists run PARALLEL and never stream, with assembly in canonical order and failure isolated per specialist; the movement backstop and every cross-channel judgment stay with the orchestrator on the MERGED diff; the offscreen specialist is an ops surface only (no simulator); one step per Director stage, no new runtime keys |
...
| Inspecting a turn: per-perceiver views, engine repairs, concurrency | `static/js/chat.js` (`openPipeline`, `perceiverViews`, `perceiverSlice`, `renderEngineNotes`, `liveStep`), `app.py` (`pipeline_get`, `_perceiver_names`), `agents/runtime.py` (`_run_parallel_group`, `_with_engine_notes`), `agents/storage.py` (`ENGINE_NOTES_KEY`), `pipeline_context.py` (`StepTaggedWarnings`, `current_step_key`) | `tests/test_pipeline_perspectives.py`, `tests/test_engine_notes.py` — three things a JSON blob cannot show: which mind a view belongs to, that a view is missing an entire sensory channel, and that two steps ran at once. Warning attribution is by contextvar, never by list position — the parallel groups run siblings on their own threads. `_engine_notes` is stripped by `active_content` so a rerun cannot carry the engine's own repair log into a prompt; anything added to it must stay diagnostic, since it rides every archive, branch and trace as opaque content |
