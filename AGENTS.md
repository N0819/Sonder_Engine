...
| Whether a barrier can be seen/heard/walked through | `spatial.py` (`_SIGHT_BARRIERS`/`_PASSABLE_BARRIERS`/`_AMBIENT_BARRIERS`, `has_visual`, `normalize_barrier`) | `visible_adjacent_rooms`, `tests/test_see_through.py` — three separate questions; `window` passes sight only, `bars` sight+sound, `membrane` passage only (the inverse of `window`: a curtained doorway, a tent flap). `has_visual` is the ONE place sight is decided. `_SOUND_LADDER` is walked by RELATIVE steps — inserting a rung changes what its NEIGHBOURS shift onto, so `membrane` is deliberately off it (`tests/test_membrane_barrier.py`) |
