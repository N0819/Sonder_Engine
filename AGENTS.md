...
| Whether a barrier can be seen/heard/walked through | `spatial.py` (`_SIGHT_BARRIERS`/`_PASSABLE_BARRIERS`/`_AMBIENT_BARRIERS`, `has_visual`, `normalize_barrier`) | `visible_adjacent_rooms`, `tests/test_see_through.py` — three separate questions; `window` passes sight only, `bars` sight+sound. `has_visual` is the ONE place sight is decided |
| Containers you can be inside (jar/cage/crate) | entity `enclosure` + `interior_rooms` + `state.hatch`, derived in `spatial.py` (`_closed_enclosure_barrier`, dock-edge rewrite) | `tests/test_see_through.py` — a closed transparent container yields a `window` edge; `_is_carried_interior` keeps a carried container's inside out of the surrounding room's view |
