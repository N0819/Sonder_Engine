...
- Every perceptual channel is barrier-gated: sight (`_SIGHT_BARRIERS`), sound (`_AMBIENT_BARRIERS` + material), scent (`_SCENT_BARRIER_LEVELS`), and touch (containment/concealment). Touch-only perception is cause-blind: surface sensations cross, the act producing them does not. Scent's gate is complete and its channel is empty — no percept builder emits `channel="smell"` and nothing on a card, entity or `StateDiff` says what anything smells like, so the verdict `scent_level` computes is read only by tests (`docs/UNBUILT.md` §1.54).
