...
| The conduct/reflection split (post-outcome cognition, the expectation gap, choice review, held beliefs, reflection-emitted ponder) | `agents/character.py` (`reflection_step`, `reflection_loop`, `expectation_gap`, the conduct-time stash), `prompts.py` (`character_conduct_prompt`, `character_reflection_enabled`, the `_CHARACTER_*` segments — the monolithic sheet is their byte-identical recomposition and the reflection sheet reuses the writing-law segments verbatim), `schemas.py` (`CharacterReflection`, `CHARACTER_REFLECTION_FIELDS`) | `agents/runtime.py` (plan insertion, the reflection/narrator pairing, `_rehydrate_loop_results`), `commit.py` (the per-character overlay — reflection wins exactly the moved fields, conduct is the fallback, so stored turns and the off default are byte-identical; `last_choice_review` persistence), design note 23, `tests/test_character_reflection.py` — the firewall is conduct's applied to a later moment: a reflection reads that mind's OWN scrubbed `perception_outcome` view/observations, own conduct, own conduct-time ledgers, never the Director's resolution and never another mind; the expectation gap is ENGINE-computed (a prediction a model grades itself against is not a prediction) and delivered as a measure beside its texts, never a verdict; the inner fan-out copies the parent context per job, in the parent |
