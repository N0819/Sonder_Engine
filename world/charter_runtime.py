...
def ingest_public_evidence(cid, evidence_rows, scene, *, turn_id,
    ...
    ``figure_acts``. ``figures`` are the names the story stands as actors
    (membership only); ``inventory_ops`` carry the one act that is not
    speech, a thing handed over.
    """
    ...
    for charter_key, item in registry["items"].items():
        ...
        if rows:
            state, metrics = apply_public_evidence(
                state, rows, scene or {}, turn_id, labels=labels)
