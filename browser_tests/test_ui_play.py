...
def test_play_runtime_refuses_a_late_story_response(
    ...
    result = page.evaluate(
        """async base => {
          const storeModule = await import(`${base}/static/js/ui-next/store.js?release=alpha98-ui9-ff279a1d1d7f`);
          const playModule = await import(`${base}/static/js/ui-next/play-runtime.js?release=alpha98-ui9-ff279a1d1d7f`);
...
def test_500_turn_render_stays_inside_the_recorded_budget(
    ...
    result = page.evaluate(
        """async base => {
          const storeModule = await import(`${base}/static/js/ui-next/store.js?release=alpha98-ui9-ff279a1d1d7f`);
          const viewModule = await import(`${base}/static/js/ui-next/play-view.js?release=alpha98-ui9-ff279a1d1d7f`);
          const prose = await import(`${base}/static/js/ui-next/prose.js?release=alpha98-ui9-ff279a1d1d7f`);
...
def test_stream_transport_can_discard_token_history_while_delivering_events(
    ...
    result = page.evaluate(
        """async base => {
          const apiModule = await import(`${base}/static/js/ui-next/api.js?release=alpha98-ui9-ff279a1d1d7f`);
