...
def test_release_module_is_importable_without_classic_host_scripts(
    ...
    result = page.evaluate(
        """async (base) => {
          const module = await import(`${base}/static/js/ui-next/release.js?release=wp06.1`);
          return {
        ...
        """async (base) => {
          ...
          try {
            await runtime.loadRuntimeModules(async path => ({
              MODULE_RELEASE: path.includes("api.js") ? "wp01" : "wp06.1",
            }));
          } catch (error) {
            return {
              name: error.name,
...
def test_reboot_replaces_runtime_listeners_instead_of_accumulating_them(
    ...
    result = page.evaluate(
        """async (base) => {
          ...
          const importer = async () => ({ MODULE_RELEASE: "wp06.1" });
    ...
    result = page.evaluate(
        r"""async (base) => {
          const { createApiClient } = await import(
            `${base}/static/js/ui-next/api.js?release=wp06.1`
          );
          let expired = 0;
    ...
    result = page.evaluate(
        """async (base) => {
          const { parseHashRoute, serializeRoute } = await import(
            `${base}/static/js/ui-next/router.js?release=wp06.1`
          );
    ...
    result = page.evaluate(
        """async (base) => {
          const { createLocalizer } = await import(
            `${base}/static/js/ui-next/localization.js?release=wp06.1`
          );
    ...
    result = page.evaluate(
        """async (base) => {
          const { setText, appendSafeRichText } = await import(
            `${base}/static/js/ui-next/content.js?release=wp06.1`
          );
    ...
    result = page.evaluate(
        """async (base) => {
          const { createTaskService } = await import(
            `${base}/static/js/ui-next/tasks.js?release=wp06.1`
          );
          let now = 100;
          let cancelCalls = 0;
          const changes = [];
    ...
    result = page.evaluate(
        """async (base) => {
          const { createNoticeService } = await import(
            `${base}/static/js/ui-next/notices.js?release=wp06.1`
          );
    ...
    result = page.evaluate(
        """async (base) => {
          const { createDiagnostics } = await import(
            `${base}/static/js/ui-next/diagnostics.js?release=wp06.1`
          );
    ...
    result = page.evaluate(
        """async (base) => {
          const { createLocalState } = await import(
            `${base}/static/js/ui-next/storage.js?release=wp06.1`
          );
    ...
    result = page.evaluate(
        """async (base) => {
          const { submitCredentialForm } = await import(
            `${base}/static/js/ui-next/credentials.js?release=wp06.1`
          );
          const calls = [];
          const apiClient = {
    ...
    result = page.evaluate(
        """async (base) => {
          const { createSaveCoordinator, classifySaveAction } = await import(
            `${base}/static/js/ui-next/save-policy.js?release=wp06.1`
          );
          const writes = [];
    ...
    result = page.evaluate(
        """async ({ base, source }) => {
          const { createExtensionRegistry } = await import(
            `${base}/static/js/ui-next/extensions.js?release=wp06.1`
          );
          const { createV1Adapter, installV1Adapter } = await import(
            `${base}/static/js/ui-next/extensions-v1.js?release=wp06.1`
          );
          const calls = [];
          const hostState = {
            boot: { extensions: [] },
