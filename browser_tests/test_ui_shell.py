    ...
    result = page.evaluate(
        """async (base) => {
          ...
          const navigationModule = await import(
            `${base}/static/js/ui-next/navigation-state.js?release=alpha98-ui2-d5cf750f8b7e`
          );
...
def test_desktop_inspector_opens_closes_pins_and_resizes_without_covering_workspace(
    ...
    expect(pin_icon).to_have_attribute("href", "/static/assets/icons/sonder-icons.svg?release=alpha98-ui2-d5cf750f8b7e#icon-pin")
    ...
    result = page.evaluate(
        """async (base) => {
          const { createShortcutRegistry } = await import(
            `${base}/static/js/ui-next/shortcuts.js?release=alpha98-ui2-d5cf750f8b7e`
          );
          let calls = 0;
          let collision = null;
