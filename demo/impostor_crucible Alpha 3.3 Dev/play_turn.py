...
t0 = time.time()
for _ev in run_pipeline(CID, tid): pass
dur = time.time() - t0
def av(k):
...
# Info-barrier instrumentation: dump each perceiver's view so leaks are visible.
po = av("perception_outcome") or av("perception_establish") or {}
views = po.get("views") or {}
for pid_key, vw in views.items():
    print(f"VIEW[{pid_key}]:", (vw or "")[:280])
with open(os.path.join(HERE, "run_log.jsonl"), "a") as fh:
    fh.write(json.dumps({"idx": idx, "dur": round(dur), "input": pin,
                         ...
                         "views": {k:(v or "") for k,v in views.items()}}, ensure_ascii=False) + "\n")
