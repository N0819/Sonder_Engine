...
| **Off-screen world ticks** | **Partial** | The low tier is built as specified. The `stochastic` rung is a real seeded draw against standing intentions (`offscreen.stochastic_ticks` — `random.Random(seed)`, no model call, same seed same ticks) taken at scene boundaries in `commit_mapping`; the earlier shipped rung rode the mapping_commit MODEL call and its tick seed seeded nothing, so ticks were neither cheap nor replayable. The model is no longer asked for ticks at any level, and a volunteered `offscreen_events` is refused on the write path. Above it, resolution is computed per tick as importance × distance (`offscreen.resolution_for`, pure; importance derives from cast/sheet/tier/psychology/memory with a `simulation.offscreen_importance` override on top; distance is beats-to-contact over scene adjacency from the `subject_last_seen` room), and the few subjects scoring `medium` get an out-of-band profile-summary tick (`offscreen.schedule_profile_ticks` → `jobs.submit` at the commit tail — `jobs.py`'s first production consumer): one bounded call, no psychology run, a provisional-tier record validated on the write path, rollback-guarded by `base_turn` at landing, and never cancelled by a turn starting. `offscreen_log` is read at re-contact by `gaps._skeleton`/`interim_for`. Still open: `reactive` is permission only, and the full-agent `character_agent` Director-adjudicated rung is unbuilt — `docs/UNBUILT.md` §2.8 |
...
| **Living world — the five state-producers** (`docs/DESIGN_LIVING_WORLD.md`) | **Floors of A, B, D built; C and E declared** | `living_world.py` carries the settings ladder for all five (declared/built split per the `OFFSCREEN_LIFE_BUILT` idiom; all default off, opt-in per chat via `/api/chats/{cid}/living_world`). A: `routines.py` recomputes routine/entropy/occupancy at contact and `director_resolve` receives the capped `destination_residue` on movement beats — nothing ticks, nothing is stored. B: `state_diff.consequences` → `living_world.mint_consequences` (location gate, due clamp, cap 2/turn, carrier-ready payload with origin/originator/witnessed/`disposition`) → `mechanics._fire_due_events` fires deterministically with a base-revision check; the notice is presence-gated — an unwitnessed fire is state awaiting contact, never a report. D: fuses fired at ungenerated places accumulate durable `place_obligations` rows BEFORE anyone arrives (truth is ungated; the surface is), consumed only by `attach_owed_history` in the mapping seam — arrival is the earning event, pinned by test. C (rumor ledger) and E (antagonist ladder) are settings-declared and unbuilt, held for the epistemic audit; §9 of the design doc records the author's carrier/anti-protagonist constraints verbatim. The off-screen ladder is the single authority ceiling over all five: `LIVING_WORLD_REQUIRES` maps every depth to the rung it spends at (floors are `deterministic` clock-work, model-assisted ceilings are `stochastic`, E is the `character_agent` rung by name) and `effective_depth` clamps to it at read time, the ceiling folded into `living_world_config` so every gate composes both axes; the settings UI is one "World simulation" card — ceiling first, mechanisms beneath, each clamp shown, never silently applied |
