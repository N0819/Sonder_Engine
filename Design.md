...
| **Off-screen world ticks** | **Partial** | Deterministic scheduling and an `offscreen_log` exist; the world advancing meaningfully during absence is narrower than specified. What the cast may do unwatched is now a per-chat ceiling on `BehaviorController`'s ladder (`scene.OFFSCREEN_LIFE_LADDER`, set in the NPC dialogue panel), gated both where the dormant cast is offered to the model and again where ticks are written. Wiring it found the engine already emitting ungated ticks on every scene change, into a field typed loosely enough that four different shapes of it are in the live logs. `reactive` and `character_agent` are permission, not behaviour — the gate exists so the villain-with-a-clock work is opt-in on a story that asked, rather than a surprise in every running one. `offscreen_log` still has no reader: `docs/UNBUILT.md` §2.8 |
