...
def test_story_author_controls_are_native_release_coherent_modules():
    ...
    for name in MODULES:
        assert 'export const MODULE_RELEASE = "alpha98-ui2-3f44d1cc71ed";' in (TOOLS / name).read_text(
            encoding="utf-8"
        )
    family = (TOOLS.parent / "live-story-tools.js").read_text(encoding="utf-8")
    for name in MODULES[1:]:
        assert f'./story-tools/{name}?release=alpha98-ui2-3f44d1cc71ed' in family
