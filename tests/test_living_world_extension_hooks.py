...
# The two ladder classes went on 2026-09-20 with the ladder itself: Charter, the
# Writers' Room and causality bubbles superseded every approach in it, so there
# is no longer a living-world slice to report or a ladder for provisioning to
# set in one act. Everything else in this file -- identity across an archive, a
# lost generation not paid for twice, resume, the half-planted town, the honest
# failure record, the detail control, the route's guards and the cap checked
# before the query -- is the EXTENSION contract and is untouched.
