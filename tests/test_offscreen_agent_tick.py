...
class TestTheTierIsReachable:
    def test_the_commit_tail_schedules_the_rung(self):
        """A mechanism assumed live that has never run is this project's
        costliest recurring discovery. The producer must be wired into the
        one place that runs after every committed turn, beside the profile
        scheduler it parallels."""
        import inspect

        import commit

        src = inspect.getsource(commit._commit_all_locked)
        assert "schedule_agent_ticks" in src
