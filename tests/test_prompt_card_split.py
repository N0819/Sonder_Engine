...
PART_COUNT = 225   # +18 (2026-09-26): `affect_appraisal.*`, the decision
                   ...
                   # `affect_appraisal.options`. +3 the same day: which mood
                   # an event stirs and how strongly, which mood a memory
                   # stirs, and how much a concern weighs now, less the
                   # single desire question the three desires replaced.
                   ...
                   # `.check_missing_event`, `.check_missing_ledger`,
                   # `.check_wrong_ledger` and `.check_placement`, and the
                   # two that say where a flagged write's change really
                   # happens (`.check_elsewhere`, `.check_which_event`).
                   # +56 (2026-09-24): the prose contract's encoder gets its
                   ...
                   # channel left the hand -- a hand is handed a slice of the
                   # beat and cannot total one it never sees whole -- and the
                   # instructions went with the grant; the beat's span is now
                   # summed from the author's per-row `seconds`.
                   # +1 (2026-09-16): `room_vocabulary`, the place rules the
                   # spatial hand's rooms chunk and `director_establish`
                   # now share by reference instead of drifting apart.
                   # +1 (2026-09-15): social obligation lifecycle owner
                   ...
                   # CORRECTION PASS IS GONE" and no engine code named either
                   # fragment;
                   # +1 (2026-09-05): the social hand's charter_ops chunk, the
