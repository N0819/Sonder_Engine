...
``background_presences`` is keyed by the prose string. The article fold
(`_presence_identity`) heals "A Dalek"/"The Dalek", but
``_presence_identity("mara") != _presence_identity("the guard")`` -- so when
the scene entity the presence belongs to is renamed ("the guard" becomes
"Mara"), the ledger starts a fresh record under the new name: dialogue turns,
addressed_turns, blurb and promotion progress stay on the orphan, the orphan
keeps being tracked, and the two can react side by side as one body.

The fix keys identity on the scene ENTITY's stable id (records carry
``entity_id``; folding by it is unconditional, since an id denotes exactly
one body) with the name as fallback for entity-less presences (a voice
through a door). The ledger's KEYS stay display-name strings -- raw readers
(agents/director.py, agents/perception.py, world/subjects.py) consume name
lists and are documented to tolerate one beat of un-healed ledger -- and
former spellings ride in ``aka`` so mentions of the old name keep counting.
...
class TestFoldFollowsTheBody:
    ...
    def test_two_bodies_never_bind_and_never_merge(self):
        ...
        folded = _fold_duplicate_presences(presences, scene)
        assert set(folded) == {"A Dalek", "The Dalek"}
        assert not folded["A Dalek"].get("entity_id")
        assert not folded["The Dalek"].get("entity_id")
    ...
    def test_a_voice_through_a_door_still_folds_by_name_alone(self):
        ...
        folded = _fold_duplicate_presences(presences, scene)
        assert set(folded) == {"A Whisper"}
        assert sorted(folded["A Whisper"]["dialogue_turns"]) == [0, 2]
...
class TestResolveFollowsTheBody:
    def test_a_new_spelling_files_under_the_bound_record(self):
        ...
        assert _resolve_presence_name("Mara", presences, scene) == "the guard"
...
class TestTrackingSurvivesARename(object):
    def test_history_and_new_dialogue_land_on_one_record(self, temp_db):
        ...
        assert "the guard" not in presences
        assert set(presences) == {"Mara"}
        rec = presences["Mara"]
    ...
    def test_a_fresh_harvest_binds_to_its_scene_body(self, temp_db):
        ...
        presences = temp_db.wget(chat_id, "background_presences", {})
        assert presences["Old Tom"].get("entity_id") == "e7"
    ...
    def test_mentions_of_the_former_name_accrue_to_the_renamed_record(
        ...
        presences = temp_db.wget(chat_id, "background_presences", {})
        assert 5 in presences["Mara"]["mention_turns"]
