...
def test_one_door_reached_twice_in_a_beat_is_one_need(temp_db, no_retrieval):
    ...
    out = mapping.compile_world_context(ctx, nonce=0)
    # Three reaches for one room -- a declared destination and two
    # generation requests -- are ONE room to plan; the first reason stands.
    (need,) = out["planning_needs"]
    assert (need["kind"], need["subject"], need["reason"]) == (
        "room", "drowned_chapel", "declared_destination_unplanned")


def test_a_need_uid_is_stable_across_reruns_and_frames_apart(temp_db, no_retrieval):
    ...
    assert a["planning_needs"][0]["uid"] == b["planning_needs"][0]["uid"]
    assert a["planning_needs"][0]["uid"] != c["planning_needs"][0]["uid"]
