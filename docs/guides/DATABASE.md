...
- `chat_lorebooks`: attachments between chats and reusable or chat-owned books. Since 2026-09-03 a LIBRARY book (chat_id NULL) is attached BY REFERENCE -- the row points at the library book itself and nothing is copied; `origin_id` is a legacy column, NULL on every new row. Another story's book is still forked into a chat-owned copy, because it has no shared origin to overlay.
- `lore_overlays`: one story's deviation from one library entry -- `(chat_id, frame_id, entry_id)` unique, per era like the scene. A NULL field inherits the library value; a set one overrides it (`keys`, `content`, `category`, `title`, the three gate fields, `circles`, `canon_locked`, and the overlay's own vector when the text changed). `disposition` is `story_edit` (a hand edit from inside a story), `room_supersession` (the Writers' Room, through a package) or `migrated_copy` (the one-time conversion of a pre-2026-09 chat copy, `core/db._migrate_chat_copies_to_overlays`). Every reader merges it (`mind.memory_lore_entries.lore_rows`); an ADDITION made from inside a story to a library book is a canon-book entry, since there is no library row to deviate from. Checkpoints snapshot the story's overlays (`lore_overlays`) and a library book as a reference, never its entries; archives carry `lore_overlays` by `entry_uid`; a branch copies them onto its own eras. Cascades from the entry and the chat.
...
- `scene.passages`: frame-scoped, the passage record (2026-09-05,
  `docs/design/DESIGN_ROOM_FIDELITY.md` §5): `{id: {rooms: [a, b], barrier,
  name, material, width, vertical, state}}`, one record per doorway, with
  each of the two rooms' edges carrying `passage: id`. Readers resolve the
  edge's barrier, name, material and width THROUGH the record when the edge
  names one and per edge when it does not, so a scene without the key reads
  byte for byte as before; `world.spatial.sync_scene_passages` (run by the
  merge and by every World Browser write) keeps the record and both edges in
  step and mints a missing edge, and `normalize_scene_passages` drops a
  record whose rooms are not two live rooms. Rides the `scene` blob whole,
  so archive, checkpoint, branch and frame split/merge carry it without
  their own handling; a frame split that keeps one room of a pair leaves a
  record the hygiene drops in that frame. The registry projection is
  untouched: a passage is not a room. Rooms may also carry `shape:
  composite` with `parts` placed by a corner word or an origin cell `at:
  [x, y]` (§2 of the same note); absent, the grid is what it was.
...
- `fiction_worlds`, `fiction_locations`, `transit_edges`: DEPRECATED dead macro schema — nothing in the runtime pipeline reads or writes them. Only the first two are kept so old imports restore: they are in `chat_archive.WORLD_TABLES` and in the checkpoint blob. `transit_edges` is named in exactly one place outside `core/db.py` — the chat-deletion sweep in `persist/chat_delete.py` — so nothing snapshots, exports, imports or restores it, and an old archive carrying rows loses them on import. Do not repeat the three-table phrasing without that split; it is the reason a deprecated table can be dropped and nobody notices. Removal of all three is planned.
