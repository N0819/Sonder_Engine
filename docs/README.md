...
- [`DESIGN_ROOM_PRELUDE.md`](design/DESIGN_ROOM_PRELUDE.md) — the Writers' Room designs the ground, and asks first: the `location` regime and its four drafting tools in place of the one-shot `utility` town prompt, the `prelude` pause between a story being created and beginning, and what the player said reaching both later passes.
