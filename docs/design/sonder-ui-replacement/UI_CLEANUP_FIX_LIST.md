...
|---|---:|---|---|---|---|---|
| UI-001 | P1 | Shared overflow / Settings | Fixed | Desktop Settings cannot scroll. At 1440x900 the content region reported `clientHeight == scrollHeight == 1782`; at 1024x600 it reported `1787`, leaving no scroll range while the viewport clipped the page. The same overflow ownership must be verified on every independently scrolling non-Play surface rather than treating the Settings reproduction as the whole defect. | Constrain shell and content grid tracks with `minmax(0, 1fr)`, give each staged list/detail surface one named overflow owner, and repair any clipped content, body-scroll substitution, nested trap, or failed list/detail restoration found in Settings, Library, Character and Persona authoring, Story Tools, New Story, dialogs, and compact sheets. | Reach the final control by wheel and keyboard where applicable at desktop, tablet, phone, short-height, landscape, and 200-percent zoom; verify Settings at 1440x900 and 1024x600 and every other named overflow owner without horizontal overflow or lost restoration state. |
...
- `.venv\Scripts\python.exe -m pytest -q browser_tests
  --basetemp=F:\git\Sonder_Engine\.tmp\codex_ui_browser_20260823`:
  198 passed.
- `.venv\Scripts\python.exe -m pytest -q -n auto
  --basetemp=F:\git\Sonder_Engine\.tmp\codex_ui_full_20260823`:
  8,797 passed and 4 platform-specific tests skipped.
- A final post-catalog parallel rerun passed 8,796 tests with the same 4 skips
  and hit one transient Windows `os.replace` permission error in
  `test_provenance_is_recorded_at_install`; its immediate isolated serial rerun
  passed. The focused language-pack/catalog group passed all 68 tests.
...
- Same-viewport Play, Library, Settings, and Story Tools renders were reviewed
  ...
  output are temporary verification artifacts rather than repository assets.
...
The work remains uncommitted and unpushed until the explicit hold recorded on
2026-08-23 is lifted.
