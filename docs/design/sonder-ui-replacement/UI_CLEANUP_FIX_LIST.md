...
| UI-BL-03 | P1 | Fixed | Library details inherited Story Tools Compact/Rail sizing, and closing a selected detail hid the panel while its grid track remained. The row ellipsis was a separate framed button that selected the record instead of exposing actions. | Keep Story Tools sizing modes scoped to Story Tools; give Library detail the expanded readable inspector, remove the track on close, and place a bare 44 px ellipsis inside the row frame with a keyboard-owned action menu that does not select the row. | Browser contracts seed Rail, prove a Library detail width of at least 320 px with no resize control, close it with workspace reaching the viewport edge, and verify ellipsis containment, transparent presentation, unchanged route, complete Story actions, Escape close, and focus return. |
