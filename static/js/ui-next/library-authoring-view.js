...
import {
  createStoryEditor,
  createStoryImporter,
} from "./library-editors/story.js?release=alpha98-ui15-5b0f039aae29";
import {
  createPersonEditor,
  createPersonImporter,
} from "./library-editors/character-persona.js?release=alpha98-ui15-5b0f039aae29";
...
function returnToLibrary(services, focusIdentity, documentRef) {
  services.library.focusOnReturn(focusIdentity);
  const returned = services.authoring.returnToLibrary();
  if (returned && focusIdentity.startsWith("library-item:")) {
...
export function mountLibraryAuthoring(options = {}) {
  ...
  const render = (library, force = false) => {
    ...
    if (state.kind === "story" && state.mode === "import") {
      ...
      back.addEventListener("click", () => returnToLibrary(services, back.dataset.focusIdentity, documentRef));
